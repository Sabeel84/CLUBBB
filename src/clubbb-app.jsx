...
.nav-logo-mark{width:42px;height:42px;background:linear-gradient(135deg,var(--acc3) 0%,var(--acc2) 45%,var(--acc) 100%);border-radius:14px;display:flex;align-items:center;justify-content:center;flex-shrink:0;box-shadow:var(--sh-gold);transition:transform .25s cubic-bezier(.34,1.56,.64,1),box-shadow .25s}
.nav-brand:hover .nav-logo-mark{transform:scale(1.1) rotate(-4deg);box-shadow:var(--sh-gold-lg)}
.nav-logo-mark span{font-family:'Syne',sans-serif;font-size:16px;font-weight:800;color:#fff;letter-spacing:.5px}
...
.nav-mob{display:none;background:transparent;border:1.5px solid var(--line2);border-radius:10px;width:38px;height:38px;font-size:16px;cursor:pointer;color:var(--ink);transition:all .18s;align-items:center;justify-content:center}
.nav-mob:hover{background:var(--bg3)}
@media(max-width:800px){.nav{padding:0 18px;height:62px}.nav-links .nbtn:not(.kill){display:none}.nav-mob{display:flex}}

/* ── MOBILE DRAWER ───────────────────────────────────── */
.mob-drawer{display:none;position:fixed;top:62px;left:0;right:0;background:rgba(255,255,255,.97);border-bottom:1px solid var(--line);backdrop-filter:blur(24px);z-index:380;padding:14px 18px 22px;flex-direction:column;gap:4px;box-shadow:0 12px 40px rgba(0,0,0,.1);transform:translateY(-8px);opacity:0;transition:all .22s cubic-bezier(.4,0,.2,1)}
.mob-drawer.open{transform:translateY(0);opacity:1}
@media(max-width:800px){.mob-drawer{display:flex}}
...
@media(max-width:760px){.hero{padding:88px 22px 80px}.hero-blob1,.hero-blob2{display:none}.hero-title{letter-spacing:-3px}}
...
@media(max-width:800px){.page{padding:30px 18px}}
...
.sh-title{font-family:'Syne',sans-serif;font-size:clamp(28px,4vw,44px);font-weight:800;letter-spacing:-2px;color:var(--ink);line-height:1}
.sh-sub{font-size:15px;color:var(--mid);margin-top:12px;font-weight:400;line-height:1.6}
...
.stat{background:var(--bg);border:1px solid var(--line);border-radius:var(--r-xl);padding:32px 24px;text-align:center;position:relative;overflow:hidden;box-shadow:var(--sh-sm);transition:all .25s cubic-bezier(.4,0,.2,1);cursor:default}
...
.stat-n{font-family:'Syne',sans-serif;font-size:clamp(44px,8vw,68px);font-weight:800;background:linear-gradient(135deg,var(--acc),var(--acc2));-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;line-height:1;letter-spacing:-2px}
.stat-l{font-family:'Plus Jakarta Sans',sans-serif;font-size:11px;font-weight:700;letter-spacing:2.5px;text-transform:uppercase;color:var(--mid2);margin-top:8px}
...
.ava{width:48px;height:48px;background:linear-gradient(135deg,var(--acc3) 0%,var(--acc2) 50%,var(--acc) 100%);border-radius:16px;display:flex;align-items:center;justify-content:center;font-family:'Syne',sans-serif;font-size:19px;font-weight:800;color:#fff;flex-shrink:0;box-shadow:0 3px 14px rgba(232,130,12,.3)}
...
.dcard-meta{display:flex;gap:16px;flex-wrap:wrap;margin-bottom:14px}
...
.club-tile-logo-init{width:56px;height:56px;border:3px solid var(--bg);border-radius:18px;background:linear-gradient(135deg,var(--acc3) 0%,var(--acc2) 50%,var(--acc) 100%);display:flex;align-items:center;justify-content:center;font-family:'Syne',sans-serif;font-size:20px;font-weight:800;color:#fff;margin-top:-38px;position:relative;z-index:2;flex-shrink:0;box-shadow:var(--sh-gold)}
        ...
        {clubs.length > 0 && <>
          ...
          <div className="clubs-grid">
            {clubs.map(cl => {
              ...
              const initials       = (cl.name || "C").split(" ").map(w => w[0]).join("").slice(0, 3).toUpperCase();
              return (
...
function Registration({ type, clubs, onReg, back }) {
  ...
  function go() {
    ...
    // Trigger email verification if hook is available
    if (window.__clubbb_hooks) {
      const hook = type === "member"
        ? window.__clubbb_hooks.onMemberRegister
        : window.__clubbb_hooks.onClubRegister;
      hook && hook(f);
      alert(`A verification email has been sent to ${f.email}. Please check your inbox and click the link to activate your account.`);
      back();
      return;
    }
    onReg(f);
      ...
      {createOpen && (
        <CreateDrive
          ...
          onSave={d => {
            ...
            if (window.__clubbb_hooks && window.__clubbb_hooks.onDrivePosted) {
              window.__clubbb_hooks.onDrivePosted(newDrive);
            }
          }}
        />
      )}
...
function ClubAdmin({ state, upd, showToast }) {
  ...
  return (
    <div className="page">
      ...
      <div className="tabs">
        {[["profile","Club Profile"],["members","Members"],["rankings","Rankings"],["promotions","Promotions"]].map(([id, l]) => (
      ...
      {tab === "members" && (
        <div style={{overflowX:"auto"}}>
          <div style={{display:"flex", gap:8, flexWrap:"wrap", marginBottom:20, alignItems:"center"}}>
            <span style={{fontFamily:"'Barlow Condensed',sans-serif", fontSize:11, letterSpacing:2, color:"var(--chalk3)", textTransform:"uppercase", marginRight:4}}>Ranks:</span>
            {myRanks.map((r, i) => (
          ...
          <table className="mtable" style={{minWidth:720}}>
            <thead>
              <tr>
                <th style={{width:44, paddingLeft:14, paddingRight:6}}>#</th>
                <th style={{minWidth:300}}>Member &amp; Rank</th>
                <th style={{width:120}}>Role</th>
                <th style={{width:90, textAlign:"center"}}>Drives</th>
                <th style={{width:190, textAlign:"center"}}>Set Rank</th>
              </tr>
            </thead>
            <tbody>
              {members.map((u, i) => (
                <tr key={u.id}>
                  <td style={{paddingLeft:14, paddingRight:6}}>
                    <span style={{fontFamily:"'Bebas Neue',sans-serif", fontSize:20, color:"var(--chalk3)"}}>{i + 1}</span>
                  </td>
                  <td style={{paddingTop:14, paddingBottom:14}}>
                    <div style={{display:"flex", alignItems:"center", gap:14}}>
                      <div className="ava" style={{width:38, height:38, fontSize:16, flexShrink:0}}>{(u.name||"?")[0]}</div>
                      <div style={{minWidth:0}}>
                        <div className="t-name" style={{marginBottom:5}}>{u.name}</div>
                        <div style={{display:"flex", alignItems:"center", gap:8, flexWrap:"wrap"}}>
                          <RankPill rankId={u.rankId} clubRanks={clubRanks} clubId={cu.clubId} />
                          <div className="t-meta" style={{margin:0}}>{u.email}</div>
                        </div>
                      </div>
                    </div>
                  </td>
                  ...
                  <td style={{textAlign:"center"}}>
                    <span style={{fontFamily:"'Bebas Neue',sans-serif", fontSize:26, color:"var(--sand)", letterSpacing:1}}>{u.drives || 0}</span>
                  </td>
                  <td style={{textAlign:"center"}}>
                    {u.id !== cu.id ? (
                      <select
                        className="fi fi-sel"
                        style={{width:"auto", padding:"5px 32px 5px 10px", fontSize:11, fontFamily:"'Barlow Condensed',sans-serif", letterSpacing:1, background:"var(--ink4)", borderBottom:"1px solid var(--sand3)", minWidth:140}}
                        value={u.rankId}
                        onChange={e => {
                      ...
                      <span style={{fontSize:11, color:"var(--chalk3)", fontFamily:"'Barlow Condensed',sans-serif", letterSpacing:1, fontStyle:"italic"}}>— YOU —</span>
                    )}
                  </td>
                </tr>
              ))}
  ...
  return (
    <div className="page">
      ...
      {tab === "clubs" && cs.map(c => {
        ...
        const cnt = us.filter(u => u.clubId === c.id && u.role !== "app_admin").length;
        return (
          <div key={c.id} className="card">
            <div style={{display:"flex", justifyContent:"space-between", flexWrap:"wrap", gap:8}}>
              <div>
                <div style={{fontFamily:"'Bebas Neue',sans-serif", fontSize:20, letterSpacing:2, color:"var(--sand)", marginBottom:6}}>{c.name}</div>
                <div style={{fontSize:12, color:"var(--chalk3)"}}>{c.email} · {c.phone}</div>
                <div style={{fontSize:12, color:"var(--chalk3)", marginTop:2}}>Admin: {adm ? adm.name : "—"}</div>
              </div>
              <span className="bdg d">{cnt} MEMBERS</span>
            </div>
            {c.description && <div style={{fontSize:13, color:"var(--chalk3)", marginTop:10}}>{c.description}</div>}
          </div>
        );
      })}

      {tab === "users" && us.filter(u => u.role !== "app_admin").map(u => {
      ...
      <div className="wrap">
        <nav className="nav">
          <div className="nav-brand" onClick={() => go(cu ? "dashboard" : "home")}>
            <div className="nav-logo-mark"><span>CB</span></div>
            <div className="nav-wordmark">CLUBBB</div>
          </div>
          ...
          <button className="nav-mob" onClick={() => setMob(o => !o)}>{mobOpen ? "✕" : "☰"}</button>
        </nav>
        <div className={`mob-drawer${mobOpen ? " open" : ""}`}>
          ...
          {cu
            ? <button className="nbtn kill" onClick={logout} style={{marginTop:8,borderTop:"1px solid var(--border)",paddingTop:14}}>Sign Out</button>
            : <>
                <button className="nbtn" onClick={() => go("login")}>Sign In</button>
                <button className="nbtn on" onClick={() => go("reg-member")}>Join</button>
              </>
